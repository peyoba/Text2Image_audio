...
import { HDImageCacheManager, authenticateImageAccess } from "./image_cache.js";
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (method === "POST" && path === "/api/auth/register") {
        ...
        logInfo(
          env,
          `[Worker Log] Processing optimize request for prompt: '${textPrompt.substring(0, 50)}...'`
        );
        ...
        if (genType === "image") {
          ...
          return jsonResponse(
            { type: genType, data: base64Image, format: "base64", content_type: "image/jpeg" },
            env
          ); // Assuming jpeg
          ...
          const audioContentType = "audio/mpeg"; // Default for mp3
        ...
        try {
          ...
          console.error(
            `[Worker Error] An unexpected error occurred in translation fetch: ${e.message}`
          );
          return jsonResponse({ translated_text: text, translated: false }, env);
        }
      } else {
...
原始描述：${textPrompt}`;
  ...
  try {
    ...
    if (
      optimizedTextRaw.length > optimizedText.length + 10 &&
      textPrompt.match(/[\u4e00-\u9fff]/)
    ) {
      console.warn(
    ...
    console.error(e.stack);
    return {
      error: `优化API调用失败 (JS fetch): ${e.message}`,
      optimized_text: textPrompt,
      raw_optimized: textPrompt,
      original_prompt: textPrompt,
    };
...
async function generateImageFromPollinations(
  ...
  if (!imageApiBase) {
    console.error(
      "[Worker Error] Image API base URL not provided in env.POLLINATIONS_IMAGE_API_BASE"
    );
...
async function generateAudioFromPollinations(
  prompt,
  env,
  voice = "nova",
  model = "openai-audio",
  ...
  if (!textApiBase) {
    console.error(
      "[Worker Error] Text API base URL not provided in env.POLLINATIONS_TEXT_API_BASE"
    );
  ...
  if (response.status === 200 && actualContentType.includes("audio/")) {
    ...
    if (audioDataArrayBuffer.byteLength < 100) {
      logInfo(
        env,
        `[Worker Warning] Pollinations API 返回的音频内容很小 (length: ${audioDataArrayBuffer.byteLength})。`
      );
    }
    ...
    logInfo(
      env,
      `[Worker Error] Pollinations API 返回非音频内容。Content-Type: ${actualContentType}, 内容片段: ${errorTextContent.substring(0, 500)}...`
    );
...
async function fetchWithRetry(url, options, apiName, env, maxRetries = 8, initialDelay = 1500) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.error(
        `[Worker Error] 调用 ${apiName} 时发生错误 (尝试 #${attempt}/${maxRetries}): ${e.message}`
      );
...
function makeCorsResponse(request, env) {
...
function addCorsHeaders(responseHeaders, env, request) {
  // responseHeaders should be a Headers object
