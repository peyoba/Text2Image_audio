 ...
 */
async function generateImageWithPremiumApi(
  prompt,
  env,
  width,
  height,
...
export async function generateAudioFromPollinations(
  ...
  if (!apiToken) {
    throw new Error(
      "语音生成功能需要 Pollen 积分。请访问 https://enter.pollinations.ai 购买积分后配置 API Key。图片生成功能仍可免费使用。"
    );
  }
  ...
  logInfo(
    env,
    `[Worker Log] Pollinations Audio API response: ${response.status}, Content-Type: ${contentType}`
  );
