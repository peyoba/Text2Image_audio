...
class ApiClient {
  ...
  setBaseUrl(url) {
    if (typeof url === "string" && url.trim()) {
      ...
      try {
        if (typeof localStorage !== "undefined") localStorage.setItem("api_base", this.baseUrl);
      } catch (e) {}
  ...
  async submitGenerationTask(text, type, options = {}) {
    ...
    console.log(
      `ApiClient: Submitting task to ${requestUrl} - Type: ${type}, Text: ${text.substring(0, 50)}..., Options:`,
      options
    );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorDetails = {
            error: "无法解析错误详情",
            details: await response.text().catch(() => "无法读取错误文本"),
          };
        }
        console.error(
          "ApiClient: Submit task error - Response not OK",
          response.status,
          errorDetails
        );
      ...
      if (type === "image") {
        ...
        console.log(
          "ApiClient: Audio task completed, received ArrayBuffer length:",
  ...
  async getTaskStatus(taskStatusUrl) {
    ...
    const fullUrl = taskStatusUrl.startsWith("http")
      ? taskStatusUrl
      : `${this.baseUrl}/tasks/${taskStatusUrl.split("/").pop()}`;
      ...
      const poll = async () => {
        try {
          ...
          if (taskInfo.status === "SUCCESS" || taskInfo.status === "FAILURE") {
            console.log(
              `ApiClient: Task ${taskInfo.task_id} polling complete. Status: ${taskInfo.status}`
            );
            resolve(taskInfo); // 任务完成 (成功或失败)
          } else if (attempts >= this.maxPollingAttempts) {
            console.warn(
              `ApiClient: Task polling reached max attempts (${this.maxPollingAttempts}).`
            );
  ...
  async generateImageWithPollinations(prompt, options = {}) {
    try {
      const { model = "flux", width = 1024, height = 1024, seed = -1, nologo = true } = options;

      console.log(
        `ApiClient: Generating image with Pollinations - Prompt: ${prompt.substring(0, 50)}..., Model: ${model}, Size: ${width}x${height}`
      );
  ...
  getAvailableImageModels() {
    return [
      // 免费模型 - 无限使用，仅有速率限制
      { id: "flux", name: "FLUX Schnell", description: "免费高质量图像生成", isFree: true, group: "free" },
      { id: "zimage", name: "Z-Image Turbo", description: "快速2倍放大", isFree: true, group: "free" },
      { id: "turbo", name: "SDXL Turbo", description: "单步快速生成", isFree: true, group: "free" },
      // FLUX 系列付费模型
      { id: "kontext", name: "FLUX.1 Kontext", description: "图像编辑", isFree: false, group: "flux" },
      { id: "klein", name: "FLUX.2 Klein 4B", description: "快速编辑 (NEW)", isFree: false, group: "flux" },
      { id: "klein-large", name: "FLUX.2 Klein 9B", description: "高质量编辑 (NEW)", isFree: false, group: "flux" },
      // GPT Image 系列
      { id: "gptimage", name: "GPT Image 1 Mini", description: "OpenAI 图像生成", isFree: false, group: "gpt" },
      { id: "gptimage-large", name: "GPT Image 1.5", description: "高级图像生成", isFree: false, group: "gpt" },
      // 其他高级模型
      { id: "nanobanana", name: "NanoBanana", description: "Gemini 图像生成", isFree: false, group: "premium" },
      { id: "nanobanana-pro", name: "NanoBanana Pro", description: "4K思考模式", isFree: false, group: "premium" },
      { id: "seedream", name: "Seedream 4.0", description: "字节跳动模型", isFree: false, group: "premium" },
      { id: "seedream-pro", name: "Seedream 4.5 Pro", description: "4K多图生成", isFree: false, group: "premium" },
    ];
  }
  ...
  async generateVoice(options) {
    try {
      ...
      console.log(
        `ApiClient: Generating voice - Text: ${text.substring(0, 50)}..., Voice: ${voice}, Speed: ${speed}`
      );
      ...
      if (lowerMime.includes("mpeg") || lowerMime.includes("mp3")) fileExtension = "mp3";
        ...
        lowerMime.includes("wave") ||
        lowerMime.includes("x-wav")
      )
        fileExtension = "wav";
