  ...
  function applyAndMaybeAutoGenerate() {
    try {
      ...
      if (text) {
        ...
        if (textInput) {
          try { textInput.value = decodeURIComponent(text); } catch (_) { textInput.value = text; }
          try { textInput.dispatchEvent(new Event('input')); } catch (_) {}
          if (source === 'main' && window.VoiceApp && typeof window.VoiceApp.showInfo === 'function') {
            try { window.VoiceApp.showInfo('已自动填入您在主页输入的文本，您可以直接生成语音或进行修改。'); } catch (_) {}
          }
          try { textInput.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (_) {}
        }
      }
      ...
      if (speed) {
        ...
        if (speedSlider) {
          ...
          if (speedDisplay) speedDisplay.textContent = String(parseFloat(speedSlider.value) || 1.0) + 'x';
          var audio = document.getElementById('generated-audio');
          if (audio) { try { audio.playbackRate = parseFloat(speedSlider.value) || 1.0; } catch (_) {} }
        }
      }
      ...
      if (auto === '1' && text && window.VoiceApp && typeof window.VoiceApp.generateVoice === 'function') {
        try { window.VoiceApp.generateVoice(); } catch (_) {}
        setTimeout(function () {
          try { window.history.replaceState({}, document.title, window.location.pathname); } catch (_) {}
        }, 0);
      }
