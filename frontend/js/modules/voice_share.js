...
(function () {
  ...
  async function share(input) {
    ...
    const text = typeof data.text === 'string' ? data.text : '我用 AISTONE 生成了一段 AI 语音，快来听听吧！';
    ...
    if (window.UIUtils && typeof window.UIUtils.copyText === 'function') {
      try { await window.UIUtils.copyText(url); return; } catch (_) {}
    }
    ...
    if (window.UIUtils && typeof window.UIUtils.toast === 'function') {
      try { window.UIUtils.toast((window.getCurrentLang && window.getCurrentLang()==='zh') ? '页面链接已复制到剪贴板' : 'Link copied to clipboard', 'success'); } catch (_) {}
    }
  }
