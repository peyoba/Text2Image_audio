...
(function () {
  ...
  function toast(msg, type) {
    if (window.UIUtils && typeof window.UIUtils.toast === 'function') {
      try { window.UIUtils.toast(String(msg || ''), type || 'info'); return; } catch (_) {}
    }
    try { console.log('[VoiceSave]', type || 'info', msg); } catch (_) {}
  }
  ...
  async function save(options) {
    ...
    if (!audioUrl) { toast('没有可保存的音频文件', 'error'); return; }
    ...
    try {
      if (window.APIClient && typeof window.APIClient.saveAudio === 'function') {
        var res = await window.APIClient.saveAudio({ audioUrl: audioUrl, text: text, voice: voice, speed: speed });
        if (res && res.success) { toast('音频已保存到个人中心', 'success'); return; }
