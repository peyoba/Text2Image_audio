...
(function () {
  ...
  function addImageActions(container, imageUrl) {
    ...
    downloadBtn.textContent =
      window.getCurrentLang && window.getCurrentLang() === "zh" ? "⬇️ 下载图片" : "⬇️ Download";
    ...
    viewBtn.onclick = function () {
      showImageModal(imageUrl, 1);
    };

    var copyBtn = document.createElement("button");
    copyBtn.className = "image-action-btn";
    copyBtn.textContent =
      window.getCurrentLang && window.getCurrentLang() === "zh" ? "📋 复制链接" : "📋 Copy Link";
    copyBtn.onclick = function () {
      copyImageData(imageUrl);
    };
  ...
  function showSingle(imageUrl) {
    ...
    img.onload = function () {
      ...
      container.appendChild(
        createInfo(
          (window.t ? t("imageInfoSize") + ": " : "Size: ") +
            width +
            " × " +
            height +
            (window.t ? " " + t("pixels") + " | " : " | ") +
            (window.t ? t("imageInfoFileSize") + ": ~" : "≈") +
            approxSizeKb +
            "KB"
        )
      );
  ...
  function showMultiple(imageUrls) {
    ...
    container.appendChild(
      createInfo(
        window.t ? t("imageInfoCount").replace("{count}", valid.length) : "Count: " + valid.length
      )
    );
    ...
    downloadAllBtn.textContent =
      window.getCurrentLang && window.getCurrentLang() === "zh" ? "⬇️ 下载全部" : "⬇️ Download All";
    ...
    gridBtn.textContent =
      window.getCurrentLang && window.getCurrentLang() === "zh" ? "🏢 网格查看" : "🏢 Grid View";
    gridBtn.onclick = function () {
      showImageModal(valid[0], 1);
    };
  ...
  function downloadAll(imageUrls) {
    ...
    var msg =
      window.getCurrentLang && window.getCurrentLang() === "zh"
        ? "\uD83D\uDCC1 开始下载 " + imageUrls.length + " 张图片"
        : "\uD83D\uDCC1 Started download of " + imageUrls.length + " images";
    if (window.UIUtils && typeof window.UIUtils.toast === "function") {
      try {
        window.UIUtils.toast(msg, "success");
      } catch (_) {}
  ...
  function showImageModal(imageUrl, index) {
    ...
    if (typeof window.showImageModal === "function") {
      try {
        return window.showImageModal(imageUrl, index);
      } catch (_) {}
    }
    // Minimal fallback modal
    var modal = document.createElement("div");
    modal.style.cssText =
      "position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.9);display:flex;justify-content:center;align-items:center;z-index:1000;cursor:pointer;";
    var img = document.createElement("img");
    img.src = imageUrl;
    img.style.cssText =
      "max-width:90vw;max-height:90vh;object-fit:contain;border-radius:8px;box-shadow:0 10px 50px rgba(0,0,0,0.5);";
    var close = document.createElement("div");
    close.textContent = "✕";
    close.style.cssText =
      "position:absolute;top:20px;right:30px;color:var(--color-surface-on-light-white, #fff);font-size:30px;font-weight:bold;user-select:none;";
    modal.appendChild(img);
    modal.appendChild(close);
    modal.onclick = function (e) {
      if (e.target === modal || e.target === close) document.body.removeChild(modal);
    };
  ...
  function copyImageData(imageUrl) {
    ...
    if (window.UIUtils && typeof window.UIUtils.copyText === "function") {
      try {
        return window.UIUtils.copyText(String(imageUrl || ""));
      } catch (_) {}
    }
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard
        ...
        .then(function () {
          ...
          if (window.UIUtils && typeof window.UIUtils.toast === "function") {
            try {
              window.UIUtils.toast(ok, "success");
            } catch (_) {}
          } else if (window.uiEnhancements && window.uiEnhancements.updateResultStatus) {
        ...
        .catch(function () {
          ...
          if (window.UIUtils && typeof window.UIUtils.toast === "function") {
            try {
              window.UIUtils.toast(fail, "error");
            } catch (_) {}
          } else if (window.uiEnhancements && window.uiEnhancements.updateResultStatus) {
