...
(function () {
  ...
  function composeFilename(base, ext) {
    const safeBase = (typeof base === 'string' && base.trim()) ? base.trim() : 'aistone_voice_' + Date.now();
  ...
  async function download(input) {
    ...
    if (window.UIUtils && typeof window.UIUtils.toast === 'function') {
      try { window.UIUtils.toast((window.getCurrentLang && window.getCurrentLang()==='zh') ? '音频下载已开始' : 'Download started', 'success'); } catch (_) {}
    }
  }
