...
class AuthManager {
    ...
    try {
      ...
      if (response.ok && result && result.success && result.token) {
        ...
        return {
          success: true,
          message:
            getCurrentLang && getCurrentLang() === "zh" ? "登录成功！" : "Logged in successfully",
        };
      } else {
        return {
          success: false,
          message:
            (result && result.error) ||
            (getCurrentLang && getCurrentLang() === "zh" ? "登录失败" : "Login failed"),
        };
      }
    } catch (error) {
  ...
  logout() {
    ...
    // 显示登出成功消息
    this.showMessage(
      getCurrentLang && getCurrentLang() === "zh" ? "已成功登出" : "Logged out successfully",
      "success"
    );
  }
    ...
    try {
      ...
      if (response.ok && result && result.success) {
        ...
        return {
          ...
          message:
            (result && result.error) ||
            (getCurrentLang && getCurrentLang() === "zh" ? "重置失败" : "Reset failed"),
        };
      }
    } catch (error) {
  ...
  getToken() {
    ...
    if (tokenFromCookie) {
      try {
        localStorage.setItem(this.tokenKey, tokenFromCookie);
      } catch (_) {}
    }
